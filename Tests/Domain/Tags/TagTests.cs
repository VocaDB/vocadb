...
public class TagTests
{
	...
	{
		Invoking(() => _tag.SetParent(_tag)).Should().Throw<ArgumentException>();
	}
	...
	{
		_tag.SetParent(_tag2);
		Invoking(() => _tag2.SetParent(_tag)).Should().Throw<ArgumentException>(); // Circular parenthood not allowed
	}
