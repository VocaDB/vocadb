...
public static class EntryWithTagsQueryableExtensions
{
	...
	public static IQueryable<TEntry> WhereNotHasTag<TEntry, TTagLink>(this IQueryable<TEntry> query, int tagId, bool childTags = false)
		where TEntry : IEntryWithTags<TTagLink>
		where TTagLink : TagUsage
	{
		if (tagId == 0)
	...
	/// <param name="childTags">Whether to search by child tags as well. Maximum of 3 levels will be searched.</param>
	/// <returns>Filtered query. Cannot be null.</returns>
	public static IQueryable<TEntry> WhereNotHasTags<TEntry, TTagLink>(this IQueryable<TEntry> query, int[]? tagIds, bool childTags = false)
		where TEntry : IEntryWithTags<TTagLink>
		where TTagLink : TagUsage
	{
		if (tagIds == null || !tagIds.Any())
		...
		return tagIds.Aggregate(query, (q, t) => WhereNotHasTag<TEntry, TTagLink>(q, t, childTags));
	}
}
