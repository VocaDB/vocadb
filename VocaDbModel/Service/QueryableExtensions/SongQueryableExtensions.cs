...
public static class SongQueryableExtensions
{
	...
	public static IQueryable<Song> WhereHasLyrics(this IQueryable<Song> query, string[]? languageCodes, bool any)
	{
		...
		else if (languageCodes != null && languageCodes.Any())
		{
			...
			{
				query = query.Where(s => s.Lyrics.Where(l => l.CultureCodes.Any(c => c.CultureCode == languageCode)).Any());
			}
			return query;
		}
		else
	...
	public static IQueryable<Song> WhereHasVocalist(this IQueryable<Song> query, SongVocalistSelection vocalist) => vocalist switch
	{
		...
		SongVocalistSelection.Other =>
			query.Where(s => s.AllArtists.Any(a => !a.IsSupport && (a.Artist.ArtistType == ArtistType.CeVIO || a.Artist.ArtistType == ArtistType.OtherVoiceSynthesizer || a.Artist.ArtistType == ArtistType.SynthesizerV || a.Artist.ArtistType == ArtistType.NEUTRINO || a.Artist.ArtistType == ArtistType.VoiSona || a.Artist.ArtistType == ArtistType.NewType || a.Artist.ArtistType == ArtistType.Voiceroid || a.Artist.ArtistType == ArtistType.VOICEVOX || a.Artist.ArtistType == ArtistType.ACEVirtualSinger || a.Artist.ArtistType == ArtistType.AIVOICE))),
		_ => query,
	};
	...
	public static IQueryable<Song> WhereHasLanguages(this IQueryable<Song> query, string[]? languages)
	{
