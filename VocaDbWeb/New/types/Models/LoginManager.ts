...
import { IEntryWithStatus } from '@/types/Models/IEntryWithStatus';
...
export enum PermissionToken {
	...
	ViewOtherPVs = 'c1bfdb49-d77e-4876-a94d-b51107593ea6',
}
...
export class LoginManager {
	...
	hasPermission = (token: PermissionToken): boolean => {
		...
		if (token === PermissionToken.ManageDatabase && this.lockdownEnabled) return false;
	...
	get canViewOtherPVs(): boolean {
		return this.hasPermission(PermissionToken.ViewOtherPVs);
	}
	...
	private static readonly normalStatusPermissions = [EntryStatus.Draft, EntryStatus.Finished];
	...
	canDeleteComment = (comment: CommentContract): boolean => {
		return (
			...
			(!!comment.author && !!this.loggedUser && comment.author.id === this.loggedUser.id)
		);
	};
	...
	allowedEntryStatuses = (entry?: EntryRefContract): EntryStatus[] => {
		...
		if (this.isDirectlyVerifiedFor(entry)) return LoginManager.trustedStatusPermissions;
	...
	canEditSongList = (songList: SongListContract): boolean => {
		if (songList.featuredCategory !== 'Nothing' && this.canEditFeaturedLists) return true;
