import { AlbumDetailsContract } from '@/types/DataContracts/Album/AlbumDetailsContract';
...
import { MediaType, PurchaseStatus } from '@/types/DataContracts/User/AlbumForUserForApiContract';
...
export class AlbumDisc {
	...
	constructor(
		...
		this.isVideo = !!discProperties && discProperties.mediaType === DiscMediaType.Video;
		this.name = discProperties?.name;
		this.totalLengthSeconds = this.songs.every((s) => s.song && s.song.lengthSeconds > 0)
			? this.songs.reduce((curr, s) => (curr += s.song!.lengthSeconds), 0)
			: 0;
	}
}
...
export class AlbumDetailsForApi {
	...
	constructor(readonly contract: AlbumDetailsContract, primaryPV: PVContract | undefined) {
		...
		this.cultureCodes = contract.songs.flatMap((s) => s.computedCultureCodes ?? []);
		...
		const songsByDiscs = groupBy(contract.songs, (s) => s.discNumber);
		this.discs = Object.entries(songsByDiscs).map(
			([key, value]) =>
				new AlbumDisc(
					Number(key),
					value,
					key in contract.discs ? contract.discs[key] : undefined
				)
		);
	...
	get releaseDateIsInTheFarFuture(): boolean {
		return !!this.fullReleaseDate && this.fullReleaseDate > dayjs.utc().add(7, 'd').toDate();
	}
	...
	get releaseDateIsInThePast(): boolean {
		return !!this.fullReleaseDate && this.fullReleaseDate <= dayjs.utc().toDate();
	}
