...
export class ArtistHelper {
	...
	static isProducerRoleType(
		...
		if (focus === ContentFocus.Video) res = res || roles.includes(ArtistRoles.Animator);
	...
	static isValidForPersonalDescription(artistLink: ArtistForAlbumContract): boolean {
	...
	static isVocalistRoleType(artistType: ArtistType, roles: ArtistRoles[]): boolean {
		// eslint-disable-next-line react-hooks/rules-of-hooks
		if (ArtistHelper.useDefaultRoles(artistType, roles)) {
			return ArtistHelper.isVocalistType(artistType);
		}

		var res = roles.includes(ArtistRoles.Vocalist) || roles.includes(ArtistRoles.Chorus);
	...
	private static useDefaultRoles(artistType: ArtistType, roles: ArtistRoles[]): boolean {
		return (
			roles == null ||
