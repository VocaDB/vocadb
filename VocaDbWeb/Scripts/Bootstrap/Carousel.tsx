...
> = React.forwardRef<CarouselRef, CarouselProps>((uncontrolledProps, ref) => {
	...
	const indicatorOnClicks = useMemo(
		() =>
			indicators &&
			Array.from({ length: numChildren }, (_, index) => (event: any): void => {
	...
	return (
		...
		>
			...
			{controls && (
				<>
					{(wrap || activeIndex !== 0) && (
						<Anchor className="carousel-control left" href="#" onClick={prev}>
							{prevIcon}
						</Anchor>
					)}
					{(wrap || activeIndex !== numChildren - 1) && (
						<Anchor className="carousel-control right" href="#" onClick={next}>
							{nextIcon}
						</Anchor>
					)}
				</>
