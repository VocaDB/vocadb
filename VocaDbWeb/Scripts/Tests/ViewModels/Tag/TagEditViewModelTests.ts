...
//namespace vdb.tests.viewModels.tags {
	...
	QUnit.test("hasTargetType - get - not set", () => {
		QUnit.equal(viewModel.hasTargetType(EntryType.Album)(), false, "hasTargetType");
	});

	QUnit.test("hasTargetType - get - is set", () => {
		QUnit.equal(viewModel.hasTargetType(EntryType.Artist)(), true, "hasTargetType");
	});

	QUnit.test("hasTargetType - set true", () => {
		viewModel.hasTargetType(EntryType.Album)(true);
		QUnit.equal(viewModel.targets(), EntryType.Album | EntryType.Artist, "targets");
		QUnit.equal(viewModel.hasTargetType(EntryType.Album)(), true, "hasTargetType");
	});
