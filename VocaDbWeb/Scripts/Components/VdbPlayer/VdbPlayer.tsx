...
const PlayerCenterControls = observer((): React.ReactElement => {
		...
		<ButtonGroup>
			...
			{vdbPlayer.playing ? (
				...
				<Button
					variant="inverse"
					title={`Play${
						vdbPlayer.canAutoplay ? '' : ' (Unavailable for this video service)'
					}`} /* LOC */
					onClick={handlePlay}
					disabled={!vdbPlayer.canAutoplay}
				>
...
const EntryInfo = observer(({ entry }: EntryInfoProps): React.ReactElement => {
	return (
...
const PlayerRightControls = observer((): React.ReactElement => {
...
const PlayerControls = observer((): React.ReactElement => {
	const { playQueue } = useVdbPlayer();
...
const EmbedPVWrapper = observer(({ pv }: PVPlayerProps): React.ReactElement => {
	const diva = useNostalgicDiva();
	const vdbPlayer = useVdbPlayer();
	const playQueue = usePlayQueue();
	...
	const handleEnded = React.useCallback(async () => {
		VdbPlayerConsole.debug(`Playback ended (repeat mode: ${playQueue.repeat})`);
...
const BottomBar = observer((): React.ReactElement => {
	const vdbPlayer = useVdbPlayer();
	...
	React.useEffect(() => {
		document.body.style.paddingBottom = `${
			(vdbPlayer.songleWidgetEnabled ? songleWidgetHeight : 0) + bottomBarHeight
		}px`;
...
export const VdbPlayer = observer((): React.ReactElement => {
