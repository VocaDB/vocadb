...
const AdminManageTagMappings = observer((): React.ReactElement => {
	const vdb = useVdb();
	...
	return (
		...
		>
			<Alert variant="info">
				...
				video streaming service. Currently only NicoNicoDouga is supported. Tags
				will be mapped from the source system (NND) to target tag on {siteName}.
				Multiple tags from the source system may be mapped to a single tag on{' '}
				{siteName}, but one source tag can be mapped to only one target tag
