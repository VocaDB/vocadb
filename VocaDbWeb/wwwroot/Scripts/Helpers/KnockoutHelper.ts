...
export default class KnockoutHelper {
	...
	public static bpm(observable: KnockoutObservable<number>): KnockoutComputed<string> {
		return ko.computed({
			read: () => {
				var val: any = observable();
			...
			write: (val: string) => {
				observable(val ? new Decimal(val).mul(1000).toInteger().toNumber() : null);
			}
		});
	}

	public static lengthFormatted(observable: KnockoutObservable<number>): KnockoutComputed<string> {
		return ko.computed({
			read: () => {
				var val: any = observable();
