...
import EntryStatus from '../../Models/EntryStatus';
	...
	function testValidate(expectedLessComplete: boolean, expectedNewer: boolean, baseStatus: EntryStatus, targetStatus: EntryStatus, baseDate: moment.Moment, targetDate: moment.Moment) {
		var result = EntryMergeValidationHelper.validate(baseStatus, targetStatus, baseDate.toISOString(), targetDate.toISOString());
	...
	QUnit.test("target is older, no warning", () => {
		testValidate(false, false, EntryStatus.Draft, EntryStatus.Draft, moment(3939), moment(39));
	});

	QUnit.test("target is older and more complete, no warning", () => {
		testValidate(false, false, EntryStatus.Draft, EntryStatus.Finished, moment(3939), moment(39));
	});

	QUnit.test("target is newer but more complete, no warning", () => {
		testValidate(false, false, EntryStatus.Draft, EntryStatus.Finished, moment(39), moment(3939));
	});

	QUnit.test("target is newer, show warning", () => {
		testValidate(false, true, EntryStatus.Draft, EntryStatus.Draft, moment(39), moment(3939));
	});

	QUnit.test("target is older but draft, show warning", () => {
		testValidate(true, false, EntryStatus.Finished, EntryStatus.Draft, moment(3939), moment(39));
	});
